...
exports.handler = async (event, context, callback) => {
  ...
  try {
    const response = await client.query(
      q.Paginate(
        q.Union(
          q.Match(q.Index("propositions_by_country"), query.country),
          q.Match(q.Index("propositions_by_state"), query.state),
          q.Match(q.Index("propositions_by_count"), query.county),
          q.Match(q.Index("propositions_by_city"), query.city),
        )
      )
    );
